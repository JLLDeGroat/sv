...
class LoadLevelLoadingWidgetTask : public FNonAbandonableTask
{
public:
    FString InMessage;
    float InPercentage;
    ...
    LoadLevelLoadingWidgetTask(FString Message, float Percentage)
        : InMessage(Message), InPercentage(Percentage), OutComplete(false)
    {
    }
    ...
    void DoWork()
    {
        ...
        if (hudDelegates)
        {
            hudDelegates->_LevelLoadingUpdate.Broadcast(InMessage, InPercentage);
        }
