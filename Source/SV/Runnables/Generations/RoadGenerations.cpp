...
UBaseGenerations *URoadGenerations::Generate()
{
	...
	for (int i = 0; i < StartZones.Num(); i++)
		RoadAllowedList.Emplace(StartZones[i]);
	for (int i = 0; i < EndZones.Num(); i++)
		RoadAllowedList.Emplace(EndZones[i]);
	...
	FindPrimaryRouteBetweenRecursive(route, EndLocation, 85);
...
void URoadGenerations::PlaceRoad(FVector loc)
{
	...
	FGraphEventRef routeTask = FFunctionGraphTask::CreateAndDispatchWhenReady([world, loc]
																			  { auto actor = world->SpawnActor<ANaturalRoad>(loc, FRotator(0)); }, TStatId(), nullptr, ENamedThreads::GameThread);
}
...
TArray<FVector> URoadGenerations::GetAdjacentGridItems(FVector item)
{
	...
	if (IsWithinList(RoadAllowedList, left))
		result.Emplace(left);
	if (IsWithinList(RoadAllowedList, right))
		result.Emplace(right);
	if (IsWithinList(RoadAllowedList, up))
		result.Emplace(up);
	if (IsWithinList(RoadAllowedList, down))
		result.Emplace(down);
...
URoadGenerations *URoadGenerations::SetRoadGenStartAndEndLocation(FVector start, FVector end)
{
	StartLocation = start;
	EndLocation = end;
	return this;
